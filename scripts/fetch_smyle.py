"""Fetch CESM2 SMYLE monthly PSL, TREFHT and PRECT from NSF NCAR GDEX.
    ...
    python scripts/fetch_smyle.py --inits 02,08   # one or more seasons only
...
No credentials required. Layout on disk groups by seasonal hindcast first, so one
initialization is a single directory that can be fetched, verified or deleted on its
own:

    <dest>/<YYYY>-<MM>/<VAR>/b.e21.BSMYLE.f09_g17.<YYYY>-<MM>.<NNN>.cam.h0.<VAR>.<span>.nc

That is 248 init directories x 3 variables x 20 members. `--inits` restricts which
start months are fetched; the year range is `YEARS` below.
        ...
        for mm in inits:
            ...
            for n in MEMBERS:
                ...
                for v in VARS:
                    ...
                    # <init>/<var>/ -- the seasonal hindcast is the top-level group
                    out.append((BASE.format(case=case, var=v, span=sp),
                                os.path.join(dest, f"{y}-{mm}", v, name)))
    return out
...
def main():
    ...
    ap.add_argument("--inits", default=",".join(INITS),
                    help="comma-separated start months to fetch, e.g. 02,08 "
                         f"(default all of {','.join(INITS)})")
    a = ap.parse_args()

    inits = [s.strip().zfill(2) for s in a.inits.split(",") if s.strip()]
    bad_inits = [s for s in inits if s not in INITS]
    if bad_inits:
        sys.exit(f"SMYLE initializes only in {','.join(INITS)}; got {bad_inits}")

    todo = jobs(a.dest, inits)
